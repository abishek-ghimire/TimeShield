class OptionsManager {
    ...
    setupFooterInteractions() {
        ...
        bindLink(
            'reportBug',
            'https://github.com/abishekgh-6/TimeShield/issues/new',
            'Opening GitHub issues to report a bug...'
        );
        bindLink(
            'requestFeature',
            'https://github.com/abishekgh-6/TimeShield/issues/new?template=feature_request.md',
            'Opening GitHub issues to request a feature...'
        );
    }
    ...
    setupSiteManagement() {
        ...
        on('scheduleSelectAllDays', 'change', (event) => {
            this.setDayGroupChecked(scheduleDays, event.target.checked);
            this.saveScheduledBlocking();
        });
        scheduleDays.forEach((day) => on(day, 'change', () => {
            this.updateSelectAllState('scheduleSelectAllDays', scheduleDays);
            this.saveScheduledBlocking();
        }));

        on('sleepBlocking', 'change', (event) => this.toggleSleepBlocking(event.target.value));
        on('sleepStartTime', 'change', () => this.saveSleepBlocking());
        on('sleepEndTime', 'change', () => this.saveSleepBlocking());
        on('sleepSelectAllDays', 'change', (event) => {
            this.setDayGroupChecked(sleepDays, event.target.checked);
            this.saveSleepBlocking();
        });
        sleepDays.forEach((day) => on(day, 'change', () => {
            this.updateSelectAllState('sleepSelectAllDays', sleepDays);
            this.saveSleepBlocking();
        }));
        on('sleepBlockAll', 'change', () => this.saveSleepBlocking());
        on('addSleepWhitelist', 'click', () => this.addSleepWhitelist());

        on('timeLimits', 'change', (event) => this.toggleTimeLimits(event.target.value));
        ...
        on('globalLimitEnabled', 'change', (event) => this.toggleGlobalLimit(event.target.value));
        on('globalLimitMinutes', 'change', () => this.saveGlobalLimit());
        on('addGlobalLimitSite', 'click', () => this.addGlobalLimitSite());
        on('newGlobalLimitSite', 'keypress', (event) => {
            if (event.key === 'Enter') this.addGlobalLimitSite();
        });
    }
