...
test('Nuclear Mode is wired as an isolated opt-in protection feature', async () => {
    ...
    assert.doesNotMatch(popup, /id="nuclearExpand"/);
    assert.doesNotMatch(popup, /id="nuclearSetup"/);
    assert.doesNotMatch(popup, /nuclearHours/);
    assert.doesNotMatch(popup, /nuclearMinutes/);
    ...
    assert.match(popupJs, /durationSeconds = savedDurationSeconds > 0 \? savedDurationSeconds : 25 \* 60/);
