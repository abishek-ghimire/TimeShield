...
test('Nuclear Mode is wired as an isolated opt-in protection feature', async () => {
    ...
    const pauseHelper = await read('floating/pause-challenge.js');
    ...
    assert.match(worker, /requestNuclearExitChallenge/);
    assert.match(worker, /completeNuclearExitWithPassword/);
    ...
    assert.doesNotMatch(worker, /whitelist\.slice\(0, 8\)|slice\(0, 8\)/);
    ...
    assert.doesNotMatch(popup, /id="nuclearActiveView"/);
    assert.doesNotMatch(popup, /id="nuclearExitButton"/);
    assert.match(popup, /id="nuclearSetupModal"[^>]*hidden/);
    assert.match(popup, /id="nuclearHours"/);
    assert.match(popup, /id="nuclearMinutes"/);
    assert.match(popup, /id="nuclearExcludeOpenTabs"/);
    assert.match(popup, /Exclude all open tabs/);
    assert.match(popup, /id="nuclearOpenTabsSummary"/);
    assert.match(popup, /id="nuclearEntry"/);
    assert.match(popup, /keep every currently open tab available before adding individual work websites or links/);
    ...
    assert.match(popup, /always included in every Nuclear session/);
    assert.match(popup, /id="nuclearSetupStart"/);
    assert.doesNotMatch(popup, /id="nuclearCard"/);
    assert.doesNotMatch(popup, /id="nuclearWhitelist"/);
    assert.doesNotMatch(popup, /id="nuclearSiteInput"/);
    assert.match(popupJs, /bind\('nuclearToggle', \(\) => this\.handleNuclearMode\(\)\)/);
    assert.match(popupJs, /active\) return this\.openNuclearBlockPage\(\)/);
    assert.match(popupJs, /toggleLabel\.textContent = active \? this\.formatNuclearRemaining/);
    ...
    assert.match(popupJs, /const activationWhitelist = \[\.\.\.new Set\(\[\.\.\.savedWhitelist, \.\.\.sessionEntries\]\)\]/);
    assert.match(worker, /const savedWhitelist = this\.normalizeNuclearWhitelist\(stored\.nuclearMode\?\.whitelist\)/);
    assert.match(worker, /\.\.\.savedWhitelist, \.\.\.candidateSites/);
    ...
    assert.match(popupJs, /Settings whitelist entries are automatically included in every new session/);
    assert.match(popupJs, /durationSeconds = \(hours \* 3600\) \+ \(minutes \* 60\)/);
    assert.doesNotMatch(popup, /nuclearSchedule/);
    assert.doesNotMatch(popupJs, /scheduleNuclearMode|clearNuclearModeSchedule|saveNuclearSchedule/);
    assert.doesNotMatch(options, /Up to 8 domains are allowed/);
    assert.doesNotMatch(optionsJs, /Nuclear Mode allows up to 8 sites/);
    assert.doesNotMatch(popupJs, /combined Nuclear whitelist is full/);
    assert.match(popupJs, /Add at least one allowed site in Settings or add an additional entry, or choose Exclude all open tabs/);
    assert.doesNotMatch(popupJs, /25-minute/);
    assert.doesNotMatch(popupJs, /savedDurationSeconds > 0 \? savedDurationSeconds : 25 \* 60/);
    assert.match(popupJs, /Add at least one allowed site in Settings or add an additional entry, or choose Exclude all open tabs/);
    ...
    assert.match(popupCss, /#nuclearToggle\.nuclear-active/);
    assert.match(popupCss, /\.nuclear-active-row \{[\s\S]*display: none !important/);
...
test('Notification sounds respect settings and fire once per timer or limit threshold', async () => {
    ...
    assert.match(blocker, /applyYouTubeShortsOnlyStyles\(\)/);
    assert.doesNotMatch(blocker, /applyYouTubeShortsFilter/);
    assert.doesNotMatch(blocker, /new MutationObserver/);
    ...
    assert.doesNotMatch(blocker, /shortsFilterNodes/);
    assert.doesNotMatch(blocker, /timeshieldHiddenShorts/);
    assert.match(blocker, /a\[href\^="\/shorts/);
    assert.match(blocker, /a\[href\^="\/shorts"\]/);
    assert.match(blocker, /ytd-reel-item-renderer/);
    assert.match(blocker, /style\.textContent = `/);
