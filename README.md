...
[![Unique Visitors ](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fabishekgh-6%2FTimeShield%2Fmain%2Fdata%2Frepository-traffic-badge.json)](https://github.com/abishekgh-6/TimeShield/blob/main/data/repository-traffic.json)
...
| **Open-tab exclusion** | Select **Exclude all open tabs** before activation to keep every tab already open in the browser available for that session, including websites, exact links, localhost pages, local files, PDFs, and other browser-open content. New tabs are still blocked unless allowlisted. |
| **Current-tab capture** | Add the currently open website or local file directly from the activation dialog when only selected work destinations should remain available. |
...
| **Strict enforcement** | Without open-tab exclusion, only entries listed for that session bypass Nuclear Mode, except that localhost pages, local files, and PDFs remain available automatically. With open-tab exclusion enabled, the captured tab IDs also bypass the block; newly opened unlisted tabs remain blocked. |
...
Nuclear Mode does not activate automatically. Clicking the control-panel button opens a fresh setup dialog with an empty active allowlist. The first choice is whether to **Exclude all open tabs**; when selected, the extension captures the current tab IDs immediately before activation so websites, links, localhost pages, local files, PDFs, and other content already open remain available. New tabs are still blocked unless explicitly allowlisted. If open-tab exclusion is not selected, work websites and exact links must be entered or deliberately selected from the saved-entry suggestions. Localhost pages, local files, and PDFs are automatic exceptions and do not need to be added. A positive duration and either one explicit website/link or the open-tab exclusion choice are required before the save-work warning can be confirmed. There is no direct Stop button in the control panel; ending an active session manually must begin from the Nuclear block page and pass verification.
